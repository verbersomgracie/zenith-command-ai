...
-- Conversation history: Admin can view all
CREATE POLICY "Admins can view all conversations"
ON public.conversation_history
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- WhatsApp messages: Admin can view all
CREATE POLICY "Admins can view all whatsapp messages"
ON public.whatsapp_messages
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- Audit logs: Admin can view all
CREATE POLICY "Admins can view all audit logs"
ON public.audit_logs
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- Daily routines: Admin can view all
CREATE POLICY "Admins can view all routines"
ON public.daily_routines
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- Routine completions: Admin can view all
